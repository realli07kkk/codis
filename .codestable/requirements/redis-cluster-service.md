...
- 作为值班人员，我希望能从当前 product 内 Redis 8 Codis Server 拉取本机当前 `dbfilename` 的已有 RDB 备份文件，并直接在 dashboard 里进入 RDB Analysis，而不是登录机器复制文件、手动上传或触发新的 `SAVE` / `BGSAVE`。
...
dashboard 可以作为当前 product 内 Redis 8 RDB HTTP export 的受控调用方。remote fetch API 只接受当前 topom model group server 中精确匹配的 `server_addr`，不接受任意 URL、path、query 或跨 product server；dashboard 用本地 `rdb_analysis_remote_fetch_auth` 发送 `X-Codis-RDB-Auth`，完整下载到受控临时文件后才创建 RDB Analysis job。
...
- Redis Stream 支持是 proxy 命令路由子集，不是完整 Redis Stream 支持声明。`XGROUP HELP` / `XINFO HELP`、带 `BLOCK` 的 `XREAD` / `XREADGROUP`、跨 hash tag multi-stream 和未知 Stream subcommand 会被 proxy 拒绝；后端仍是 Redis 3 时，proxy 只保证按 stream key 路由，最终 unknown command 由后端返回。该能力不包含 Vector Set、Hash field expiration、RedisJSON、RedisBloom、RedisTimeSeries、RediSearch 或 Redis Stack 模块能力。
...
- RDB Analysis 只分析已有 RDB 文件；它不是在线 keyspace 实时订阅，不替代 Redis `INFO`/stats，也不自动生成 RDB。Remote fetch 只允许 dashboard 从当前 product 内 Redis 8 server 的固定 RDB HTTP export 拉取已有 `dbfilename` 文件，不读取 Redis Server 任意路径，不支持远端主动上传、tus、Range、断点续传或 replication stream 抓取 RDB。分析结果可能包含 key 名，只通过 dashboard `xauth` API 暴露，且 dashboard 重启后任务消失。
- RDB HTTP export 只导出 Redis 8 Codis Server 本机当前 `dbfilename` 对应的已有 RDB；它不是 RDB Analysis，不分析 key，不主动上传到 dashboard，不支持客户端指定路径、Range、压缩、加密、限速或独立 HTTP 端口，也不替代网络隔离、TLS 或外层访问控制。两个 `codis-rdb-export-*` 配置均重启生效。Dashboard remote fetch 复用该 export 时只使用 dashboard 持久化 auth，目标 server 必须属于当前 product model。
