...
- Redis 8 Codis Server 目前仍是独立支线：`make codis-server-redis8` 可编译 Redis 8 二进制，`--codis-enabled yes` 可启动 Codis mode，并通过基础 slot 命令、同步迁移命令和 `SLOTSRESTORE` 验证 Redis 8 ↔ Redis 8 的 1024 slot keyspace、tag index 与 RDB fragment restore；默认 `make` / `make codis-server` 仍构建 Redis 3 Codis Server。异步迁移、Go 组件适配和正式默认产物切换仍等待 `redis8-upgrade` 后续条目完成。
