...
- **Hot key cache**：Codis Proxy 进程内、默认关闭、按 exact key 配置的短 TTL string 读缓存；只作用于稳定 slot 上的 `GET` / `MGET` 完整 bulk value，不进入 coordinator，也不做跨 proxy 失效广播。
...
- Hot key cache 是 opt-in bounded-stale 能力：默认关闭，只缓存配置 allowlist 中 exact key 的 `GET` / `MGET` string bulk value；同 proxy 写请求在后端响应完成后清理本地 cache，跨 proxy 或直连后端 Redis 的写入只能靠 `hot_key_cache_ttl` 收敛。slot 迁移或锁定状态不从 cache 返回，slot 映射更新会失效对应 slot 的本地条目。首版不做自动热点探测、dashboard 管理页、coordinator 元数据、跨 proxy 失效广播，也不缓存派生读或非 string value。
